...
def treeSpec(agent):
	...
	spec = [Selector, KillHero, ChaseHero]
...
class ChaseHero(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.target == None or self.target.isAlive() == False:
			...
			print("exec", self.id, "false")
			return False
		elif distance(self.agent.getLocation(), self.target.getLocation()) < self.agent.getMaxRadius() * AREAEFFECTRANGE:
			# succeeded
			self.agent.dodge()
			print("exec", self.id, "true")
			return True
...
class KillHero(BTNode):
	...
	def execute(self, delta = 0):
		ret = BTNode.execute(self, delta)
		if self.target == None or distance(self.agent.getLocation(), self.target.getLocation()) > self.agent.getMaxRadius() * AREAEFFECTRANGE:
			# failed executability conditions
			if self.target == None:
				print("foo none")
			else:
