...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        //TO DO: deal with the case where the end of the level is before the end of the chunk?
        //double check the level length stuff...

        while (i < width - EXITOFFSET) {
            this.setBlock(i, DEFAULTHEIGHT-2, HILL_TOP);
            this.setBlock(i, DEFAULTHEIGHT-1, GROUND);
            char chunk_block = Character.valueOf(dna.getChromosome().charAt(dna_i));
            int chunk_len = Integer.valueOf(dna.getChromosome().charAt(dna_i + 1));
            switch (chunk_block) {
            case 'C':
                createCoin(i, chunk_len);
            case 'S':
                createStair(i, chunk_len);
            case 'G':
                createGap(i, chunk_len);
            case 'E':
                createEnemy(i, chunk_len);
            default:
                createGround(i, chunk_len);
            }
            i += chunk_len;
            dna_i += 2;
        }
    ...
    // helper functions
    public void createCoin(int start, int length) {
	    for (int i = start; i < start + length; i++) {
	        this.setBlock(i, DEFAULTHEIGHT - 3, COIN);
        }
    }

    public void createStair(int start, int length) {
	    for (int i = start; i < start + length; i++) {
	        int height = DEFAULTHEIGHT - i - 1;
	        for (int j = 1; j < height; j++) {
	            this.setBlock(i, DEFAULTHEIGHT - (2 + j), GROUND);
            }
	        this.setBlock(i, height, HILL_TOP_LEFT);
        }
    }

    public void createGap(int start, int length) {
	    for (int i = start + 1; i < start + length; i += 2) {
	        this.setBlock(i, DEFAULTHEIGHT-2, EMPTY);
	        this.setBlock(i, DEFAULTHEIGHT-1, EMPTY);
	    }
    }

    public void createEnemy(int start, int length) {
	    for (int i = start; i < start + length; i++) {
	        if (i % 3 == 0) {
	            this.setSpriteTemplate(i, DEFAULTHEIGHT - 3, new SpriteTemplate(2, false));
            }
	        if (i % 5 == 0) {
	            this.setSpriteTemplate(i, DEFAULTHEIGHT - 3, new SpriteTemplate(1, true));
            }
        }
    }

    public void createGround(int start, int length) {
	    for (int i = start; i < start + length; i++) {
	        this.setBlock(i, DEFAULTHEIGHT - 2, HILL_TOP);
	        this.setBlock(i, DEFAULTHEIGHT-1, GROUND);
        }
